...
{ 	
 	...
 	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOG, ENABLE);	 //使能PC,D,G端口时钟
...
{
  	unsigned char character=1,Line=1,List=0;
		OLED_Clear();
		OLED_ShowCHinese(0,1,0);
		delay_ms(100);
		for(character=1;character<21;character++)
		{
		  OLED_ShowCHinese(List+=16,Line,character);
			delay_ms(100);
			if((character+1)%8==0)
			{
			 Line+=2;
